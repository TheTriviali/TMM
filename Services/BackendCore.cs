...
namespace TMM
{
    ...
    public class BackendCore
    {
        ...
        /// <summary>Get loadouts directory for a game profile.</summary>
        private string GetLoadoutsPath(GameProfile profile)
            => Path.Combine(AppDataPath, profile.RawFolderName, "loadouts");

        /// <summary>Get list of available loadouts for a game.</summary>
        public List<string> GetAvailableLoadouts(GameProfile profile)
        {
            string dir = GetLoadoutsPath(profile);
            if (!Directory.Exists(dir)) return new();

            return Directory.GetFiles(dir, "*.json")
                .Select(f => Path.GetFileNameWithoutExtension(f))
                .OrderBy(n => n == "Default" ? 0 : 1)  // Default first
                .ToList();
        }

        /// <summary>Save current mod state as a named loadout.</summary>
        public async Task SaveLoadoutAsync(GameProfile profile, string loadoutName, IEnumerable<ModItem> mods)
        {
            string dir = GetLoadoutsPath(profile);
            ...
            string filePath = Path.Combine(dir, $"{loadoutName}.json");
            var json = JsonSerializer.Serialize(loadout, JsonHelper.PrettyOptions);
            await File.WriteAllTextAsync(filePath, json);
        }

        /// <summary>Load a saved loadout, returning the mod state.</summary>
        public async Task<ModLoadout?> LoadLoadoutAsync(GameProfile profile, string loadoutName)
        {
            string dir = GetLoadoutsPath(profile);
            string filePath = Path.Combine(dir, $"{loadoutName}.json");
            ...
            try
            {
                ...
                return JsonSerializer.Deserialize<ModLoadout>(json, JsonHelper.PrettyOptions);
            }
            catch { return null; }
        }

        /// <summary>Delete a saved loadout.</summary>
        public async Task DeleteLoadoutAsync(GameProfile profile, string loadoutName)
        {
            string dir = GetLoadoutsPath(profile);
            string filePath = Path.Combine(dir, $"{loadoutName}.json");
